...
public class RssSaxParser extends DefaultHandler
{
	...
	private List<MPEvent> aEvents = new ArrayList<>();
			...
			throws SAXException
	{
		...
		if (pQualifiedName.equalsIgnoreCase(ELEMENT_ITEM))
		{
			if (!this.aInHeader)
			{
				this.aEvents.add(createMPEvent(this.aCurrentTitle, this.aCurrentLink, this.aCurrentDescription, this.aCurrentDate)); 
				this.aCurrentTitle = "";
				this.aCurrentLink = "";
				this.aCurrentDescription = "";
				this.aCurrentDate = "";
			}
			this.aInHeader = false;
			return;
		}
		...
		{
			this.aCurrentTag = pQualifiedName;
		}
	}
	...
	public void characters(char[] pCharacters, int pStart, int pLength) throws SAXException
	{
		...
		if (this.aCurrentTag.equalsIgnoreCase(ELEMENT_TITLE))
		{
			this.aCurrentTitle = this.aCurrentTitle + createNewString(pCharacters, pStart, pLength);
		}
		else if (this.aCurrentTag.equalsIgnoreCase(ELEMENT_LINK))
		{
			this.aCurrentLink = this.aCurrentLink + createNewString(pCharacters, pStart, pLength);
		}
		else if (this.aCurrentTag.equalsIgnoreCase(ELEMENT_DESCRIPTION))
		{
			this.aCurrentDescription = this.aCurrentDescription + createNewString(pCharacters, pStart, pLength);
		}
		else if (this.aCurrentTag.equalsIgnoreCase(ELEMENT_DATE))
		{			
			this.aCurrentDate = this.aCurrentDate + createNewString(pCharacters, pStart, pLength);
		}
	}
